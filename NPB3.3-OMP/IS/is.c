...
#define PAGE_ADDR(addr) (((unsigned long)addr)&PAGE_MASK)
#define SPLITSZ (MAX_KEY/SPLITS)
#define SUBSPLITSZ (MAX_KEY/(SPLITS*numa))
#define NUMA_OFF(node,split) (split*SPLITSZ+node*SUBSPLITSZ)
#include <numa.h>
#endif //NUMA
...
/* allocates a zone */
struct ccontrol_zone1 * ccontrol_new(void);
struct ccontrol_zone2 * ccontrol_new(void);
...
ccontrol_create_zone(ccontrol_zone1, c_set1, MAX_KEY*sizeof(INT_TYPE));
ccontrol_create_zone(ccontrol_zone1, c_set2,
        (MAX_KEY+SIZE_OF_BUFFERS)*sizeof(INT_TYPE));


// TODO:
// ccontrol_free(key_array, ccontrol_zone2);
// ccontrol_free(key_buff2, ccontrol_zone2);
// ccontrol_free(key_buff1, ccontrol_zone1);
//ccontrol_delete(ccontrol_zone2);
//ccontrol_destroy_zone(ccontrol_zone2);
//ccontrol_delete(ccontrol_zone1);
//ccontrol_destroy_zone(ccontrol_zone1);


    key_array=(INT_TYPE*)ccontrol_malloc(ccontrol_zone2,SIZE_OF_BUFFERS*sizeof(INT_TYPE));
    key_buff1=(INT_TYPE*)ccontrol_malloc(ccontrol_zone1,MAX_KEY*sizeof(INT_TYPE));
    key_buff2=(INT_TYPE*)ccontrol_malloc(ccontrol_zone2,SIZE_OF_BUFFERS*sizeof(INT_TYPE));
...
#ifdef TABARNAC
    #pragma omp for private(i,j,k,k1) schedule(static,NUM_BUCKETS/(SPLITS*omp_get_max_threads()))
#else
#ifdef SCHED_CYCLIC
...
#ifdef TABARNAC
    #pragma omp for schedule(static,NUM_BUCKETS/(SPLITS*omp_get_max_threads()))
#else
#ifdef SCHED_CYCLIC
...
// This loop is responsible for the gaussian behaviour
...
/*  Printout initial NPB info */
    ...
    if(argc < 1)
    {
	    printf("argv[1] must be the number of numa nodes\n");
	    exit(1);
    }
    numa=atoi(argv[1]);
    //printf(" Running on %d cores on %d NUMA nodes\n", omp_get_max_threads(),numa);
    if(!(key_array=(INT_TYPE*)numa_alloc(SIZE_OF_BUFFERS*sizeof(INT_TYPE))))
    {
        perror("Memory allocation error");
        exit(1);
    }
    if(!(key_buff1=(INT_TYPE*)numa_alloc(MAX_KEY*sizeof(INT_TYPE))))
    {
        perror("Memory allocation error");
        exit(1);
    }
    if(!(key_buff2=(INT_TYPE*)numa_alloc_interleaved(SIZE_OF_BUFFERS*sizeof(INT_TYPE))))
    {
        perror("Memory allocation error");
        exit(1);
    }
    //printf("Splitsz %d, subsplitsz %d MAX %d\n", SPLITS, SUBSPLITSZ, MAX_KEY);
    for(node=0; node< numa; ++node)
    {
        numa_tonode_memory(key_array+node*SIZE_OF_BUFFERS/numa, (SIZE_OF_BUFFERS/numa)*sizeof(INT_TYPE), node);
        for(i=0; i< SPLITS;++i)
        {
	    numa_tonode_memory((void *)PAGE_ADDR(key_buff1+NUMA_OFF(node,i)), SUBSPLITSZ*sizeof(INT_TYPE),node);
            //printf("key_buff1 %d: -> %d to node %d\n", NUMA_OFF(node,i), NUMA_OFF(node,i)+SUBSPLITSZ,node);
        }
    }
#endif
